...
__weak void MCboot( MCI_Handle_t* pMCIList[NBR_OF_MOTORS] )
{
  ...
  else
  {
    ...
    /*
     * USART1 已用于右侧触觉传感器，当前不启动 Motor Pilot/ASPEP。
     * 恢复 Motor Pilot 时需要切换到独立通信口，不能复用触觉串口。
     */
    ...
    (void)RCM_RegisterRegConv(&BusCurrentRegConv_M1);
...
LL_GPIO_LockPin(M1_BUS_CURRENT_GPIO_Port, M1_BUS_CURRENT_Pin);
...
LL_GPIO_LockPin(KTH7812_SCK_GPIO_Port, KTH7812_SCK_Pin);
LL_GPIO_LockPin(KTH7812_MISO_GPIO_Port, KTH7812_MISO_Pin);
LL_GPIO_LockPin(KTH7812_MOSI_GPIO_Port, KTH7812_MOSI_Pin);
LL_GPIO_LockPin(KTH7812_NSS_GPIO_Port, KTH7812_NSS_Pin);
}
